...
def get_url(base_url):
    ...
    base_url=base_url+'search?categories=010&purity=100&ratios=landscape&topRange=1w&sorting=toplist&order=desc'
...
def download_img(img_url_list):
    ...
    for i in range(len(img_url_list)): 			#循环遍历列表，对每张壁纸缩略图的url进行字符串的增删获得壁纸原图下载的url  注：jpg或png结尾
        ...
        with open(img_path,'wb') as fp:		    #('w':写入,'b':二进制格式)
            fp.write(img_data)
            print(img_name,'下载成功')     	
        randimgs.write('//jihulab.com/zknb-666/Random-ACG-API/-/raw/wallhavenrequests/pc/'+str(img_name)+'\n')
        print("链接:\n"+str(img_url)+"\n获取完成\n")
    randimgs.close()
